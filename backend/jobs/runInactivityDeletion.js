#!/usr/bin/env node
/**
 * Standalone Inactivity Deletion Job Runner
 *
 ...
 *   0 2 * * * cd /path/to/backend && node jobs/runInactivityDeletion.js >> logs/inactivity-deletion.log 2>&1
 *
 * Features:
...
const { processInactivityDeletions } = require('./inactivityDeletion');
...
async function main() {
  ...
  console.log('========================================');
  console.log(`[CRON] Inactivity Deletion Job Started`);
  ...
  try {
    ...
    // Execute inactivity deletion process
    console.log('[CRON] Starting inactivity deletion process...');
    const results = await processInactivityDeletions();
    ...
    console.log('========================================');
    console.log('[CRON] Inactivity Deletion Job Completed');
    console.log(`[CRON] Duration: ${duration}ms`);
    console.log(`[CRON] Results:`);
    console.log(`[CRON]   - Users processed: ${results.success}`);

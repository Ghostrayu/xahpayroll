...
**DECENTRALIZED HOURLY PAYROLL ON THE XAHAU LEDGER**
...
**[DOCUMENTATION](DOCUMENTS/) • [QUICKSTART](DOCUMENTS/QUICKSTART.md) • [SECURITY](SECURITY.md) • [CONTRIBUTING](CONTRIBUTING.md)**
...
**XAH PAYROLL** IS A PRODUCTION-READY, OPEN SOURCE DECENTRALIZED PAYROLL MANAGEMENT SYSTEM THAT ENABLES AUTOMATIC WAGE PAYMENTS IN XAHAU USING PAYMENT CHANNELS ON THE XRP LEDGER.

THE SYSTEM PROVIDES NGO/EMPLOYERS WITH TRANSPARENT ESCROW MANAGEMENT, WORKERS WITH RELIABLE PAYMENTS, AND LEVERAGES BLOCKCHAIN TECHNOLOGY FOR TAMPER-PROOF VERIFICATION AND AUTOMATED FUND DISTRIBUTION.

### 🎯 WHY XAH PAYROLL?

#### FOR WORKERS
- **💰 GET PAID EVERY HOUR**: RECEIVE XAH DIRECTLY TO YOUR WALLET AUTOMATICALLY
- **🔒 GUARANTEED PAYMENT**: FUNDS LOCKED IN ON-CHAIN ESCROW BEFORE WORK STARTS
- **📱 SIMPLE INTERFACE**: CLOCK IN/OUT WITH XAMAN WALLET - NO COMPLEX SETUP
- **🌍 BORDERLESS**: WORK FROM ANYWHERE, GET PAID ANYWHERE

#### FOR NGOS & EMPLOYERS
- **⏱️ PAY AS YOU GO**: NO UPFRONT SALARY COMMITMENTS - ESCROW ONLY WHAT YOU NEED
- **📊 TRANSPARENT TRACKING**: REAL-TIME MONITORING OF WORK HOURS AND BALANCE ACCUMULATION
- **🛡️ FRAUD PREVENTION**: ON-CHAIN VERIFICATION PREVENTS TIMESHEET MANIPULATION
- **💼 MULTI-WORKER MANAGEMENT**: SUPPORT MULTIPLE WORKERS ACROSS ORGANIZATIONS

#### FOR THE ECOSYSTEM
- **🌐 DECENTRALIZED**: NO CENTRALIZED CONTROL - RUNS ON XAHAU LEDGER
- **🔓 OPEN SOURCE**: FULLY TRANSPARENT CODE UNDER MIT LICENSE
- **🔐 SECURE**: ENTERPRISE-GRADE XAMAN WALLET INTEGRATION
- **⚡ EFFICIENT**: MINIMAL TRANSACTION FEES WITH OFF-CHAIN BALANCE ACCUMULATION

## CORE FEATURES

### CORE FUNCTIONALITY
- **HOURLY WAGE TRACKING**: WORKERS TRACK HOURS WORKED WITH AUTOMATIC WAGE CALCULATION
- **SINGLE PAYMENT SETTLEMENT**: ACCUMULATED EARNINGS PAID IN ONE TRANSACTION WHEN CHANNEL CLOSES
- **REAL-TIME BALANCE TRACKING**: LIVE MONITORING OF WORK HOURS AND ACCUMULATED BALANCE
...
### 🕐 TWO-FIELD BALANCE ARCHITECTURE
...
1. NGO/EMPLOYER CREATES PAYMENT CHANNEL
   ...
   - CHANNEL ID RETRIEVED VIA 3-TIER FALLBACK SYSTEM (TX QUERY → ACCOUNT_CHANNELS → RETRY)
...
3. OFF-CHAIN BALANCE ACCUMULATION
   - ON CLOCK-OUT: off_chain_accumulated_balance += (HOURS_WORKED × HOURLY_RATE)
...
4. CHANNEL CLOSURE REQUEST (NGO OR WORKER)
   ...
   - STATUS → 'CLOSING' WITH SETTLEDELAY PROTECTION
...
6. TRANSACTION VERIFICATION & DATABASE SYNC
   ...
   - VERIFY: CHANNEL REMOVED FROM LEDGER (ACCOUNT_CHANNELS QUERY → entryNotFound)
   - DATABASE: off_chain_accumulated_balance → 0, STATUS → 'CLOSED', last_ledger_sync → NOW()
...
> 2. **NAVIGATE TO EACH DASHBOARD**: WORKER, NGO, PAYMENT CHANNEL SCREENS
> 3. **CAPTURE SCREENSHOTS**: USE YOUR OPERATING SYSTEM'S SCREENSHOT TOOL
> 4. **SAVE TO**: `DOCS/SCREENSHOTS/` DIRECTORY (CREATE IF DOESN'T EXIST)
> 5. **NAME FILES**:
>    - `WORKER-DASHBOARD.PNG`
>    - `NGO-DASHBOARD.PNG`
>    - `PAYMENT-CHANNEL-CREATION.PNG`
>    - `WORK-SESSION-TRACKING.PNG`
> 6. **UPDATE README**: REPLACE PLACEHOLDERS BELOW WITH ACTUAL IMAGE REFERENCES

### WORKER DASHBOARD
![WORKER DASHBOARD](DOCS/SCREENSHOTS/WORKER-DASHBOARD.PNG)
*WORKERS CAN CLOCK IN/OUT, VIEW PAYMENT CHANNELS ACROSS ORGANIZATIONS, AND TRACK ACCUMULATED BALANCES*

### NGO/EMPLOYER DASHBOARD
![NGO DASHBOARD](DOCS/SCREENSHOTS/NGO-DASHBOARD.PNG)
*NGOS MANAGE WORKERS, CREATE PAYMENT CHANNELS, AND MONITOR ACTIVE PAYMENTS IN REAL-TIME*

### PAYMENT CHANNEL CREATION
![PAYMENT CHANNEL CREATION](DOCS/SCREENSHOTS/PAYMENT-CHANNEL-CREATION.PNG)
*CREATE PAYMENT CHANNELS WITH WORKER SELECTION, HOURLY RATE CONFIGURATION, AND ESCROW FUNDING*

### WORK SESSION TRACKING
![WORK SESSION TRACKING](DOCS/SCREENSHOTS/WORK-SESSION-TRACKING.PNG)
*REAL-TIME TRACKING OF WORK HOURS WITH AUTOMATIC WAGE CALCULATIONS AND BALANCE ACCUMULATION*
...
### CORE FEATURES
- **🔐 SECURE ESCROW**: FUNDS LOCKED ON-CHAIN BEFORE WORK STARTS
- **📱 XAMAN WALLET INTEGRATION**: ENTERPRISE-GRADE WALLET WITH QR CODE SCANNING
- **👥 MULTI-ORGANIZATION SUPPORT**: WORKERS CAN WORK FOR MULTIPLE NGOS/EMPLOYERS
- **📊 REAL-TIME TRACKING**: LIVE MONITORING OF WORK HOURS AND ACCUMULATED BALANCE
- **🔄 PAYMENT CHANNELS**: EFFICIENT OFF-CHAIN BALANCE ACCUMULATION WITH ON-CHAIN SETTLEMENT
- **🛡️ WORKER PROTECTION**: SETTLEDELAY PERIOD ENSURES WORKERS CAN CLAIM EARNED FUNDS
- **🌐 NETWORK FLEXIBILITY**: SUPPORTS BOTH XAHAU TESTNET AND MAINNET

### WORKER CAPABILITIES
- CLOCK IN/OUT WITH XAMAN WALLET AUTHENTICATION
- VIEW ALL PAYMENT CHANNELS ACROSS MULTIPLE EMPLOYERS
- TRACK ACCUMULATED BALANCE AND ESCROW STATUS IN REAL-TIME
- WORKER-INITIATED CHANNEL CLOSURE WITH BALANCE PROTECTION
- COMPLETE PAYMENT HISTORY AND TRANSACTION RECORDS

### NGO/EMPLOYER CAPABILITIES
- ADD WORKERS WITH XAMAN QR CODE SCANNING OR MANUAL WALLET ENTRY
- CREATE PAYMENT CHANNELS WITH HOURLY RATE CONFIGURATION
- FUND CHANNELS WITH XAH ESCROW (PAY-AS-YOU-GO MODEL)
- MONITOR ACTIVE CHANNELS AND WORKER STATUS
- VIEW PAYMENT ANALYTICS AND ORGANIZATIONAL STATISTICS
- CANCEL CHANNELS WITH AUTOMATIC ESCROW RETURN
...
- XAMAN WALLET (DOWNLOAD AT HTTPS://XAMAN.APP)
```
...
6. USE NGO DASHBOARD TO MONITOR WORKER HOURS, ACCUMULATED BALANCES, ESCROW STATUS, AND CHANNEL ANALYTICS.
...
**KEY FEATURES:**
- ✅ SAME WORKER CAN WORK FOR MULTIPLE ORGANIZATIONS
- ✅ SAME WORKER CAN HAVE MULTIPLE PAYMENT CHANNELS (DIFFERENT JOBS)
- ✅ WORKER ADDRESSES VALIDATED AUTOMATICALLY
- ✅ PREVENTS NGO/EMPLOYER WALLETS FROM BEING ADDED AS WORKERS
- ✅ QR CODE SCANNING ELIMINATES ADDRESS ENTRY ERRORS
...
5. **SIGN TRANSACTION**
   - XAMAN WALLET WILL PROMPT FOR SIGNATURE (QR CODE OR DEEP LINK)
...
**IMPORTANT NOTES:**
- ✅ ONLY THE CHANNEL OWNER (NGO/EMPLOYER) CAN CANCEL
- ✅ WORKER RECEIVES ALL ACCUMULATED UNPAID BALANCE
- ✅ UNUSED ESCROW AUTOMATICALLY RETURNS TO YOUR WALLET
- ✅ CANCELLATION IS PERMANENT AND CANNOT BE UNDONE
- ✅ XAMAN WALLET INTEGRATION FOR SECURE TRANSACTION SIGNING
- ⚠️ CANNOT CANCEL ALREADY-CLOSED CHANNELS
...
**KEY FEATURES:**
- ✅ **SOFT DELETE WITH 48-HOUR GRACE PERIOD**: REVERSIBLE DELETION WITH ACCOUNT RECOVERY
- ✅ **HARD DELETE AFTER GRACE PERIOD**: PERMANENT DATA REMOVAL VIA SCHEDULED JOBS
- ✅ **AUTOMATIC INACTIVITY DELETION**: WORKERS INACTIVE FOR 14+ DAYS AUTO-DELETED
- ✅ **PDF DATA EXPORT**: DOWNLOAD ALL PERSONAL DATA BEFORE DELETION (GDPR RIGHT TO PORTABILITY)
- ✅ **ORGANIZATION NOTIFICATIONS**: NGOS NOTIFIED WHEN WORKERS DELETE PROFILES
- ✅ **SAFETY CHECKS**: CANNOT DELETE WITH ACTIVE PAYMENT CHANNELS OR UNPAID BALANCES
- ✅ **AUDIT TRAIL**: COMPLETE DELETION LOGS FOR COMPLIANCE AND DEBUGGING
...
**DELETION BLOCKED IF:**
- ❌ ACTIVE PAYMENT CHANNELS EXIST
- ❌ UNPAID BALANCE > 0 XAH
...
- ✅ 48-HOUR BUFFER PREVENTS ACCIDENTAL DELETION
...
- **DELETION CRITERIA (OR LOGIC):**
  1. **INSTANT DELETION:** DELETED ACCOUNTS WITH NO ACTIVE CHANNELS/UNPAID BALANCES (ALLOWS IMMEDIATE REMOVAL)
  2. **GRACE PERIOD DELETION:** DELETED ACCOUNTS OLDER THAN 48 HOURS (SAFETY NET FOR ACCOUNTS WITH ACTIVE CHANNELS)
- **PROCESS:**
  1. QUERY SOFT-DELETED USERS MEETING DELETION CRITERIA
  2. PERMANENTLY DELETE USER DATA VIA TRANSACTION-BASED ATOMIC OPERATIONS
  3. UPDATE DELETION LOGS WITH `HARD_DELETED_AT` TIMESTAMP
  4. LOG EXECUTION RESULTS TO CONSOLE

**INACTIVITY DELETION JOB:**
- **SCHEDULE:** RUNS DAILY AT 2:00 AM
- **LOCATION:** `backend/jobs/inactivityDeletion.js`
- **CRON:** `0 2 * * *`
- **INACTIVITY THRESHOLD:** 14 DAYS (2 WEEKS) WITH NO LOGIN
- **SAFETY CHECKS:** ONLY DELETES IF NO ACTIVE CHANNELS AND NO UNPAID BALANCES
- **PROCESS:**
  1. QUERY WORKERS INACTIVE FOR 14+ DAYS
  2. SOFT-DELETE ELIGIBLE INACTIVE USERS
  3. CREATE DELETION LOG ENTRIES
  4. NOTIFY ALL ASSOCIATED ORGANIZATIONS
  5. LOG EXECUTION RESULTS TO CONSOLE

**MONITORING:**
- JOB EXECUTION LOGS WRITTEN TO CONSOLE OUTPUT
...
2. ✅ VALIDATE MIGRATION SCRIPT ON STAGING
3. ✅ RUN ALL AUTOMATED TESTS
4. ✅ COMPLETE MANUAL TESTING CHECKLIST
5. ✅ CONFIGURE SCHEDULED JOBS (CRON)
...
0 * * * * NODE /PATH/TO/BACKEND/JOBS/hardDelete.js >> /PATH/TO/LOGS/hardDelete.log 2>&1

# INACTIVITY DELETION JOB (DAILY AT 2 AM)
0 2 * * * NODE /PATH/TO/BACKEND/JOBS/inactivityDeletion.js >> /PATH/TO/LOGS/inactivityDeletion.log 2>&1
```

**MONITORING:**
- LOG OUTPUT: CONSOLE LOGS REDIRECTED TO FILES (SEE CRON SETUP ABOVE)
- METRICS: DELETION SUCCESS RATE, JOB EXECUTION TIME
- ALERTS: MONITOR LOG FILES FOR JOB FAILURES AND HIGH ERROR RATES
...
- [x] **WORKER DELETION SYSTEM** - COMPREHENSIVE PROFILE DELETION WITH GDPR COMPLIANCE ✅
...
THIS REPOSITORY IS AVAILABLE FOR VIEWING, LEARNING, AND REFERENCE. SEE [CONTRIBUTING.MD](CONTRIBUTING.MD) FOR CODE STYLE GUIDELINES AND DEVELOPMENT SETUP IF YOU'D LIKE TO RUN THE PROJECT LOCALLY.
...
COMPREHENSIVE PROJECT DOCUMENTATION IS AVAILABLE IN THE [`DOCUMENTS/`](DOCUMENTS/) FOLDER:
...
- **NETWORK CONFIGURATION**: [`DOCUMENTS/NETWORK_CONFIG.MD`](DOCUMENTS/NETWORK_CONFIG.MD)
- **OPEN SOURCE PREPARATION**: [`DOCUMENTS/OPEN_SOURCE_PREPARATION.MD`](DOCUMENTS/OPEN_SOURCE_PREPARATION.MD)
- **TESTING GUIDES**: [`DOCUMENTS/TEST_QUICK_VALIDATION.MD`](DOCUMENTS/TEST_QUICK_VALIDATION.MD), [`DOCUMENTS/TEST_COMPREHENSIVE_SUITE.MD`](DOCUMENTS/TEST_COMPREHENSIVE_SUITE.MD)
- **BACKEND DOCS**: [`DOCUMENTS/BACKEND/`](DOCUMENTS/BACKEND/)
- **FRONTEND DOCS**: [`DOCUMENTS/FRONTEND/`](DOCUMENTS/FRONTEND/)

SEE [`DOCUMENTS/README.MD`](DOCUMENTS/README.MD) FOR COMPLETE DOCUMENTATION INDEX.

## USING THIS PROJECT

THIS REPOSITORY IS AVAILABLE AS AN OPEN-SOURCE REFERENCE IMPLEMENTATION. WHILE WE ARE NOT ACCEPTING EXTERNAL CONTRIBUTIONS AT THIS TIME, YOU ARE WELCOME TO:

- **VIEW THE CODE**: LEARN FROM THE IMPLEMENTATION
- **RUN LOCALLY**: FOLLOW THE [QUICKSTART GUIDE](DOCUMENTS/QUICKSTART.MD)
- **REPORT SECURITY ISSUES**: SEE [SECURITY.MD](SECURITY.MD)
- **ASK QUESTIONS**: OPEN A GITHUB ISSUE FOR CLARIFICATION

SEE [CONTRIBUTING.MD](CONTRIBUTING.MD) FOR CODE STYLE GUIDELINES AND DEVELOPMENT SETUP INFORMATION.

## LICENSE

THIS PROJECT IS LICENSED UNDER THE MIT LICENSE - SEE THE [LICENSE](LICENSE) FILE FOR DETAILS.

**TLDR**: YOU CAN USE, MODIFY, AND DISTRIBUTE THIS SOFTWARE FREELY, INCLUDING FOR COMMERCIAL PURPOSES, AS LONG AS YOU INCLUDE THE ORIGINAL COPYRIGHT AND LICENSE NOTICE.

## SECURITY

SECURITY IS A TOP PRIORITY FOR XAH PAYROLL. IF YOU DISCOVER A SECURITY VULNERABILITY:

**DO NOT OPEN A PUBLIC ISSUE**

INSTEAD, PLEASE EMAIL **ADMIN@XAHPAYROLL.XYZ** OR CREATE A PRIVATE SECURITY ADVISORY ON GITHUB.

SEE [SECURITY.MD](SECURITY.MD) FOR FULL SECURITY POLICY, BEST PRACTICES, AND REPORTING GUIDELINES.

## COMMUNITY

- **GITHUB ISSUES**: [REPORT SECURITY ISSUES OR ASK QUESTIONS](HTTPS://GITHUB.COM/YOUR-ORG/XAH-PAYROLL/ISSUES)
- **EMAIL**: ADMIN@XAHPAYROLL.XYZ FOR SECURITY OR GENERAL INQUIRIES
...
SPECIAL THANKS TO ALL [CONTRIBUTORS](https://github.com/your-org/xah-payroll/graphs/contributors) WHO HELP MAKE XAH PAYROLL BETTER.
